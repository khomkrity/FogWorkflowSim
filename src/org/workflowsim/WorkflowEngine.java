...
public class WorkflowEngine extends SimEntity {
    ...
    public static ArrayList<double[]> indicators = new ArrayList<>();
    public static ArrayList<Double> updatebest = new ArrayList<>();
    ...
    public List<? extends Vm> getAllVmList() {
        if (this.vmList != null && !this.vmList.isEmpty()) {
            ...
            List<? extends Vm> list = new ArrayList<>();
...
//    	System.out.println(CloudSim.clock()+":"+CloudSim.getEntityName(ev.getSource())+"-> WorkflowEngine = "+ev.getTag());
    ...
    protected void processJobSubmit(SimEvent ev) {
        List<? extends Cloudlet> list = (List<? extends Cloudlet>) ev.getData();
        setJobsList(list);
    ...
    private void init() throws Exception {
        for (Job job : jobList) {
            ...
            for (Task task : job.getTaskList()) {
                task.initlength();
                task.setCloudletLength(task.initlength());
            }
        }
        CloudSim.clock = 0.1;
        for (int i = 0; i < getSchedulerIds().size(); i++) {
...
//      					System.out.println("*************************************************************************按照最优调度方案执行");
     ...
     * @return
     */
    private boolean hasJobListContainsID(List<Job> jobList, int id) {
        for (Job job : jobList) {
     ...
     */
    private void setSchedulers(List<FogBroker> list) {
        this.scheduler = list;
    }
     ...
     * Gets the scheduler .
     *
     * @param schedulerId scheduler index
