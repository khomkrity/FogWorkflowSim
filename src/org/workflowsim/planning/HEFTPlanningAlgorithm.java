...
public class HEFTPlanningAlgorithm extends BasePlanningAlgorithm {
	...
	private double findFinishTime(Task task, CondorVM vm, double readyTime, boolean occupySlot) {
		...
		if (sched.isEmpty()) {
			if (occupySlot) {
				sched.add(new Event(task, readyTime, readyTime + computationCost));
			}
		...
		if (sched.size() == 1) {
			...
			if (occupySlot) {
				sched.add(pos, new Event(task, start, start + computationCost));
			}
		...
		if (occupySlot) {
			sched.add(pos, new Event(task, start, finish));
		}
		return finish;
	}
	...
	private void printSchedules() {
		...
		for(Map.Entry<CondorVM, List<Event>> schedule : schedules.entrySet()) {
			...
			for(Event event : events) {
				...
				int taskId = task.getCloudletId();
				double startTime = event.start;
				double finishTime = event.finish;
