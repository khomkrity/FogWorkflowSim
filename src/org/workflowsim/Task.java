...
public class Task extends Cloudlet {
	...
	/*
	 * The depth of this task. Depth of a task is defined as the furthest path from
	 * the root task to this task. It is set during the workflow parsing stage.
	 */
	private int depth;
	/*
	...
	/*
	 * parents' task delay map (communication cost).
	 */
	private Map<Integer, Double> parentDelay;

	/**
	 * Allocates a new Task object. The task length should be greater than or equal
	 * to 1.
	 *
	...
	public Task(final int taskId, final long taskLength) {
		/**
		 * We do not use cloudletFileSize and cloudletOutputSize here. We have added a
		 * list to task and thus we don't need a cloudletFileSize or cloudletOutputSize
		 * here The utilizationModelCpu, utilizationModelRam, and utilizationModelBw are
		 * just set to be the default mode. You can change it for your own purpose.
		 */
		super(taskId, taskLength, 1, 0, 0, new UtilizationModelFull(), new UtilizationModelFull(),
		...
		this.parentDelay = new HashMap<>();
	}

	/**
	 * Sets parent's task delay (second) 
	 *
	 * @param cloudletId parent task id
	 * @param delay parent task delay (second)
	 */
	public void setParentDelay(int cloudletId, double delay) {
		parentDelay.put(cloudletId, delay);
	}
	
	/**
	 * Sets parent's task delay (the whole map) only used in processCloudletSubmit 
	 *
	 * @param parentsDelay a hash map containing all parent's task delay <Integer, Double> 
	 */
	public void setDelayMap(Map<Integer, Double> parentsDelay) {
		this.parentDelay = parentsDelay;
	}

	/**
	 * Gets all parent's task delay
	 *
	 * @return parentDelay a hash map containing cloudletId, delay
	 */
	public Map<Integer, Double> getParentDelay() {
		return this.parentDelay;
	}
...
//	}
	 ...
	 * getProcessingCost does not take cpu cost into it also the data file in Task
	 * is stored in fileList <tt>Processing Cost = input data transfer +
	 ...
	 */
	/*
	 * @Override public double getProcessingCost() { // cloudlet cost: execution
	 * cost...
	 * 
	 * double cost = getCostPerSec() * getActualCPUTime();
	 * 
	 * // ...plus input data transfer cost... long fileSize = 0; for (FileItem file
	 * : getFileList()) { fileSize += file.getSize() / Consts.MILLION; } cost +=
	 * costPerBw * fileSize; return cost; }
	 */
}
