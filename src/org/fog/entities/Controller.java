...
/**
 * Controller contains the evaluating indicators library, which can calculate
 * each indicator.
 * 
...
//				MSendTime = (getSendSize()/1024) / getmobile().getUplinkBandwidth();
	...
	public void print() {
		...
		System.out.println("Total Delay = " + FogBroker.totalDelay);
		printPowerDetails();
		printCostDetails();
		System.out
				.println("Offloading to Cloud: " + count1 + ", to Fog: " + count2 + ", and in mobile: " + count3 + ".");
			...
			if (!CloudSim.getEntity(datacenterId).getName().contains("m")) {
				for (FileItem file : job.getFileList()) {
					...
					if (file.getType() == FileType.INPUT) {
						...
						} else if (fileToDatacenter.containsKey(file.getName())
								&& !fileToDatacenter.get(file.getName()).contains(datacenterId)) {
		...
		for (Job job : jobList) {
			...
			if (!CloudSim.getEntity(datacenterId).getName().contains("m")) {
				if (id == datacenterId) {
					...
					double c = job.getActualCPUTime() * vm.getHost().getTotalMips() / vm.getMips()
							* host.getcostPerMips();
