...
public class FogDevice extends PowerDatacenter {
    ...
    public FogDevice(String name, long mips, int ram, double uplinkBandwidth, double downlinkBandwidth,
        ...
        PowerHost host = new PowerHost(hostId, costPerMips, new RamProvisionerSimple(ram),
                new BwProvisionerOverbooking(bw), storage, peList, new VmSchedulerTimeShared(peList), powerModel);
     ...
     * <b>NOTE:</b> You do not need to override {@link Datacenter#registerOtherEntity()} method, if you use
     * this method.
     *
     * @pre $none
     * @post $none
    ...
    protected void updateCloudletProcessing() {
        ...
        // this is a bug of CloudSim if the runtime is smaller than 0.1 (now is 0.01) it
        // doesn't work at all
    ...
    /**
     * Processes a Cloudlet submission. The cloudlet is actually a job which can be
     * cast to org.workflowsim.Job
     *
...
//            System.out.println("host.id:"+host.getId()+" vmId:"+vmId);
            ...
            switch (Parameters.getCostModel()) {
                case DATACENTER -> job.setResourceParameter(getId(), getCharacteristics().getCostPerSecond(),
                        getCharacteristics().getCostPerBw());
                case VM -> job.setResourceParameter(getId(), vm.getCost(), vm.getCostPerBW());
                default -> {
                }
            }
     ...
     * condor-io) add files to the local storage; For a shared file system (such as
     * NFS) add files to the shared storage
     *
     * @param job, the job
     * @pre $none
     * @post $none
            ...
            if (file.isRealInputFile(requiredFiles)) {
                ...
                switch (ReplicaCatalog.getFileSystem()) {
                    case SHARED: // 进入share
                         ...
                         */
                        /*
                         * double maxRate = Double.MIN_VALUE; for (Storage storage : getStorageList()) {
                         * double rate = storage.getMaxTransferRate(); if (rate > maxRate) { maxRate =
                         * rate; } } //Storage storage = getStorageList().get(0);
                         *
                    ...
                    case LOCAL:
                        ...
                        for (Iterator it = siteList.iterator(); it.hasNext(); ) {
                            ...
                            System.out.println("site: " + site + " this: " + this.getName());
                            ...
                            if (site.equals(Parameters.SOURCE)) {
                                ...
                                bwth = Math.min(vm.getBw(), getVmAllocationPolicy().getHost(Integer.parseInt(site), userId)
                        ...
                        // We currently don't use this storage to do anything meaningful. It is left for
                        // future.
